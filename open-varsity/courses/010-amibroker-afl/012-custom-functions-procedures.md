Chapters
Module B · The AFL Language - Chapter 12
...
AFL gives you two ways to package logic, and the difference is simple:
  * A **`function`**does a calculation and hands a value _back_ with `return`. You use it on the right-hand side of an assignment, exactly like `EMA`. `smoothed = mySmooth(Close, 10);`
  * A **`procedure`**_does_ something - plots, draws, sets a variable - but returns nothing. You call it as a standalone statement on its own line. `drawDashboard();`
...
![Calling a function: the arguments go in, the function does its work, and one value comes back out](https://openalgo.in/amibroker/images/ch12-function-call.png) DiagramCalling a function: the arguments go in, the function does its work, and one value comes back out
Key idea
...
![Arguments fill the parameters in order - the first argument lands in the first parameter, the second in the second](https://openalgo.in/amibroker/images/ch12-parameters.png) DiagramArguments fill the parameters in order - the first argument lands in the first parameter, the second in the second
`return` does two jobs at once: it sends a value back, _and_ it stops the function immediately. Anything after a `return` does not run. That makes `return` handy for early exits:
Copy
```
...
Think of `return` as the one-way door out of the function. It packs up a _single_ value, hands it back to the exact spot where the function was called, and ends the function right there - and any working variables it built along the way simply disappear:
![return hands one value back to the caller, while the local variables built inside the function vanish when it ends](https://openalgo.in/amibroker/images/ch12-return.png) Diagramreturn hands one value back to the caller, while the local variables built inside the function vanish when it ends
...
Heads up
Reach for `global` sparingly. A function that quietly changes outside variables is hard to reason about and easy to break. Prefer passing values _in_ as parameters and handing the answer _out_ with `return` - functions that only talk through their arguments are the ones you can trust and reuse without surprises.
...
When the job is to _draw_ rather than to _calculate_ , write a procedure. Here is one that marks a level with a labelled line - the kind of thing you might call several times:
Copy
```
